...
class EntropyModel(nn.Module):
    ...
    def quantize_noise(self, inputs, means=None, quantize_tensor=None):
        half = float(0.5)

        if quantize_tensor is None:
            noise = torch.empty_like(inputs).uniform_(-half, half)
        elif quantize_tensor is not None:
            if len(quantize_tensor.shape) == 3:
                quantize_tensor = torch.FloatTensor(quantize_tensor).unsqueeze(dim=0)
            elif len(quantize_tensor.shape) == 4:
                quantize_tensor = torch.FloatTensor(quantize_tensor)
            quantize_tensor = quantize_tensor.cuda(device=inputs.device)
            noise = torch.empty_like(inputs).uniform_(-half, half) * quantize_tensor
        inputs = inputs + noise
        return inputs

    def quantize_dequantize(self, inputs, means=None, quantize_tensor=None):
        outputs = inputs.clone()
        if means is not None:
            outputs -= means
        if quantize_tensor is None:
            ...
            if len(quantize_tensor.shape) == 3:
                ...
                quantize_tensor = torch.FloatTensor(quantize_tensor)
            quantize_tensor = quantize_tensor.cuda(device=outputs.device)
            quantize_tensor.requires_grad = False
    ...
    def quantize_dequantize_v2(self, inputs, means, scales, quantize_tensor=None):
        outputs = inputs.clone()
        ...
        if quantize_tensor is None:
            ...
            reconstruction_level_limit = torch.log(torch.clamp(scales, min=1e-10) * 6.1094102 * 2) / torch.log(3 * torch.ones(1, device=scales.device))
            reconstruction_level_limit = torch.clamp(torch.pow(3, torch.ceil(reconstruction_level_limit)), min=3)
            reconstruction_level_limit = torch.floor(reconstruction_level_limit / 2)

            value = outputs
            interval = quantize_tensor
            max_interval = interval.max()
            center = torch.round(value / interval) * interval
            boundary = torch.arange(start=0, end=max_interval+1, step=1, device=value.device).unsqueeze(dim=1).unsqueeze(dim=2).unsqueeze(dim=3).unsqueeze(dim=4)
            boundary = boundary / max_interval * interval + center - interval/2
            reconstruction_level_1 = (boundary[1:] + boundary[:-1])/2

            distribution = torch.distributions.normal.Normal(0,scales)
            boundary_cdf = distribution.cdf(boundary)
            reconstruction_level_1_prob = boundary_cdf[1:] - boundary_cdf[:-1]
            reconstruction_level = torch.sum(reconstruction_level_1 * (reconstruction_level_1_prob + 1e-10), dim=0) / torch.sum(reconstruction_level_1_prob + 1e-10, dim=0)

            #outputs = torch.clamp(reconstruction_level, min=-reconstruction_level_limit, max=reconstruction_level_limit)
            outputs \
                = (reconstruction_level > reconstruction_level_limit) * reconstruction_level_limit \
                + (reconstruction_level < -reconstruction_level_limit) * -reconstruction_level_limit \
                + (reconstruction_level <= reconstruction_level_limit) * (reconstruction_level_limit >= -reconstruction_level_limit) * reconstruction_level
...
class EntropyBottleneck(EntropyModel):
    ...
    *tensorflow/compression*. See the original paper and the `tensorflow
    ...
    def update(self, force: bool = False, update_quantiles: bool = False) -> bool:
        ...
        # NOTE: `update_quantiles`는 과거 API 호환성용으로만 받는다.
        # 최신 CompressAI에서는 해당 인자가 없는 버전이 있어, 상위 호출부가
        # 인자를 전달해도 무시하도록 한다.
        if update_quantiles:
            warnings.warn(
                "EntropyBottleneck.update(update_quantiles=...) is ignored; "
...
class GaussianConditional(EntropyModel):
    ...
    *tensorflow/compression*. See the `tensorflow documentation
    ...
    def _likelihood_with_quantize_tensor(
        self, inputs: Tensor, scales: Tensor, means: Optional[Tensor] = None, quantize_tensor: Optional[list] = None,
    ) -> Tensor:
        ...
        quantize_tensor = quantize_tensor.cuda(device=inputs.device)
        quantize_tensor.requires_grad = False
        half = float(0.5) * quantize_tensor
    ...
    def forward_with_quantize_tensor(
        self,
        inputs: Tensor,
        scales: Tensor,
        means: Optional[Tensor] = None,
        training: Optional[bool] = None,
        quantize_tensor = None,
    ) -> Tuple[Tensor, Tensor]:
        if training is None:
            training = self.training

        if training is True:
            outputs = self.quantize_noise(inputs, means, quantize_tensor=quantize_tensor)
        elif training is False:
            outputs = self.quantize_dequantize(inputs, means, quantize_tensor=quantize_tensor)
        likelihood = self._likelihood_with_quantize_tensor(outputs, scales, means, quantize_tensor=quantize_tensor)
        if self.use_likelihood_bound:
            likelihood = self.likelihood_lower_bound(likelihood)
        return outputs, likelihood
